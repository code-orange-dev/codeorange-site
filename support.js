...
(() => {
  ...
  function initLazyVideos() {
    ...
    const observeVideo = (video) => {
      if (!video || seen.has(video) || video.__coLazyVideoObserved) return;
    ...
    const scan = (root = document) => {
      if (root.matches?.('[data-lazy-video]')) observeVideo(root);
      root.querySelectorAll?.('[data-lazy-video]').forEach(observeVideo);
    };
    const start = () => {
      scan();
